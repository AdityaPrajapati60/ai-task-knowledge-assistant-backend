...
from agent.verification import hallucination_score, confidence_score
...
def run_agent(prompt: str, user, db):
    ...
    for step in planner_steps:
        ...
        if tool_name == "generate_answer" and context:  
            try: 

                hallucination = hallucination_score(result, context)
                confidence = confidence_score(result, context)

                run.hallucination_score = hallucination
                run.confidence_score = confidence
 
                print(f"[VERIFICATION] Hallucination Score: {hallucination}")
                print(f"[VERIFICATION] Confidence Score: {confidence}")

                # Safety Gate
                if context is not None:
                    if hallucination > 0.4 or confidence < 0.5:
                       result = "Answer confidence is low or unsupported. Escalation required."
            
            except Exception as e:
                    print(f"[VERIFICATION ERROR] {e}")
                    run.hallucination_score = None
    ...
    if context and result == context:
        result = TOOLS["generate_answer"](question=prompt, context=context)
    ...
    if context is not None and isinstance(result, str) and len(result) > 1000:
        result = TOOLS["generate_answer"](question=prompt, context=context)
